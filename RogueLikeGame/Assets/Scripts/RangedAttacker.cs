...
public class RangedAttacker : MonoBehaviour
{
    ...
    public float shotDistance = Mathf.Infinity;
    ...
    {
        //Debug.Log(path[0]);
        if (timeTilFind <= 0 /*&& (ourWorldToCell(myPlayer.transform.position) != oldPlayerPos)*/ && ourWorldToCell(transform.position) != endPos && restPath())
        {
